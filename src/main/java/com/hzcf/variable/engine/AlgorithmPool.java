...
public class AlgorithmPool {
	...
	private void setAlgorithms(Map<String,Map<String,DerivedAlgorithms>> algorithmMap){
	...
	public String readVariable(){
		...
		try {
			...
			logger.debug(String.format("读取变量完毕，WriteHoldCount[%S] ReadLockCount[%s] WriteLocked[%s] QueueLength[%s]",
			GlobalLock.reentrantReadWriteLock.getWriteHoldCount(),GlobalLock.reentrantReadWriteLock.getReadLockCount(),GlobalLock.reentrantReadWriteLock.isWriteLocked(),GlobalLock.reentrantReadWriteLock.getQueueLength()));
			//写入算法池，获取全局写锁。
			GlobalLock.writeLock.lock();
			setAlgorithms(templateAlgorithmPool);
			logger.info("变量池已更新！");
		}catch (Exception e) {
			info = "刷新变量池失败," + e.getCause();
			logger.error(info, e.fillInStackTrace());
		} finally {
